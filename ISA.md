...
updated: 2026-09-16T17:57:42Z
principal_stated_goal: "I want to make a simple web program to do AA spot check inventories based on the methods written in the big book of alcoholics anonymous on pages 86 through 88."
principal_stated_goal_source: prompt
principal_stated_goal_signal: 2
principal_stated_goal_locked: 2026-09-16T17:57:42Z
...
- Typographic register: serif question text at reading size, generous whitespace, warm off-white ground, one restrained accent, near-invisible chrome. Dark mode designed rather than derived, since the nightly review happens in a dark room. (Derived from the principal's second stated want: "I want this to be elegantly laid out and intuitive to use.")
...
- [ ] ISC-1: The repository contains `index.html`, `manifest.webmanifest`, and `sw.js`, and contains no `package.json`, lockfile, or build configuration.
- [ ] ISC-2: A push to `main` publishes to `https://fob698.github.io/spot-check-inventory/`, which returns HTTP 200.
- [ ] ISC-3: With the network disabled, a cold launch renders the tab shell and all five tabs.
- [ ] ISC-4: An entry written while offline persists and reads back after a full relaunch (after: ISC-3).
- [ ] ISC-5: Publishing a new build updates an already-installed instance within one relaunch, with no manual cache clearing.
- [ ] ISC-6: The last-selected tab is restored after full app termination.
- [ ] ISC-7: In-progress input persists on a debounced keystroke; force-quitting mid-entry and relaunching restores the unsaved text.
- [ ] ISC-8: No delete or clear action executes without a confirmation step.
- [ ] ISC-9: From the home-screen icon, any tab reaches a writable state in at most two taps.
- [ ] ISC-10: First contentful paint on a cold launch is under 1000ms on throttled Fast 3G.
- [ ] ISC-11: Dark mode follows `prefers-color-scheme`, and a manual override persists across relaunch.
- [ ] ISC-12: Anti: zero network requests originate after the app shell has loaded.
- [ ] ISC-13: Anti: no account, login, sync, amends-tracking, reminder, notification, badge, or streak surface exists anywhere in the build.
- [ ] ISC-14: Anti: no quoted passage longer than a single question phrase appears in the app or the repository, and every quoted phrase carries its page citation.
...
- [ ] ISC-17: Every long-form input in Nightly renders collapsed on a fresh open of the tab.
- [ ] ISC-18: A Nightly entry with exactly one question answered saves successfully (after: ISC-16).
- [ ] ISC-18.1: No field in Nightly blocks a save with a validation error (after: ISC-16).
...
- [ ] ISC-22: A Spot Check entry saves with exactly one question answered and the rest untouched (after: ISC-20).
...
- [ ] ISC-24: The grid captures a row and adds further rows without leaving the tab.
- [ ] ISC-25: The grid is completable one-handed at a 390px viewport with no horizontal page scroll.
...
- [ ] ISC-27: History lists saved entries newest-first, each showing its date and its type.
- [ ] ISC-28: An entry opened from History renders every answered field expanded and collapses only empty fields.
- [ ] ISC-29: Text search in History matches against entry body content, not only titles and dates.
...
- [ ] ISC-30: The info page appears automatically on first open and dismisses in one tap.
- [ ] ISC-31: An info control is present on every tab and reopens the page (after: ISC-30).
- [ ] ISC-32: The info page states that no data is held on any server and that everything remains on the device.
- [ ] ISC-33: The info page explains that adding the app to the home screen keeps entries longer, and no part of the app requires or blocks on installation.
- [ ] ISC-34: Anti: the info page states that the app is not affiliated with or approved by Alcoholics Anonymous World Services.
...
| ISC-2 | curl | HTTP status of the published URL after a push | 200 | curl | principal_stated_goal |
...
| ISC-16 | manual-review | compare rendered questions against p.86 | full set present, cited | Read | principal_stated_goal |
...
| ISC-20 | manual-review | compare rendered questions against p.84–85 | four-horsemen set present, cited | Read | principal_stated_goal |
| ISC-21 | manual-review | compare pause prompt against p.87–88 | present, cited | Read | principal_stated_goal |
| ISC-22 | manual-browser | save with one answer only | saves | Interceptor | Principles |
| ISC-23 | manual-review | compare rendered content against p.86–87 | plan and asks present, cited | Read | principal_stated_goal |
...
- 2026-09-16 — Recommended one-question-at-a-time paced flow; **overridden**. The principal has built a version of this before and found he does not usually answer every question, so pacing forces answers he does not want to give. Single scrolling form with free navigation adopted, with collapsed-by-default long-form inputs as the mechanism that keeps it from reading as a wall.
...
- 2026-09-16 — Scope grew from the requested pp. 86–88 to five tabs, adding the pp. 64–67 resentment grid and a History tab, at the principal's request. The two additions are the largest single source of build cost in this ISA.
...
_Empty. Claims close on tool evidence; each entry collapses to a one-line provenance stub._
